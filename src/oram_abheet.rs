...
impl Client {
    ...
    pub fn gen_dummy_database(&mut self) -> Vec<u64> {
        ...
        //     db.push(data_item);
        // }
        //write the created database into a file
        // let mut f = File::create_new("Plain_db.txt")
        // .expect("unable to open file !!! plaindb file alread exists.\n");
        // for i in &db{
        //     writeln!(f,"{}",i).expect("Unable to write database into memory");
        // }
        db
    }
    */
...
fn rw(
    ...
    let mut c0 = RLWECiphertext::allocate(ctx.poly_size, ctx.modulus);
...
fn update_db_mt(
    ...
    let g = |j: usize, l: &RGSWCiphertext| {
        ...
        let mut new_data: RLWECiphertext = RLWECiphertext::allocate(
            ctx.poly_size, ctx.modulus);
        l.cmux_with_buf(
            &mut new_data,
            &data.clone().read().unwrap()[j],
            &tmp,
...
fn process_one_st(
    ...
    let mut y: RLWECiphertext = RLWECiphertext::allocate(ctx.poly_size, ctx.modulus);
    let mut ls: Vec<RGSWCiphertext> = vec![
...
fn update_db_st(
    ...
    for j in 0..cols {
        ...
        let mut new_data = RLWECiphertext::allocate(ctx.poly_size, ctx.modulus);
        ls[j].cmux_with_buf(
            &mut new_data,
            &data.clone().read().unwrap()[j],
            &tmp,
